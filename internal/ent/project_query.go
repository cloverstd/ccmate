...
func (_q *ProjectQuery) QueryLabelRules() *ProjectLabelRuleQuery {
...
func (_q *ProjectQuery) WithLabelRules(opts ...func(*ProjectLabelRuleQuery)) *ProjectQuery {
	query := (&ProjectLabelRuleClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withLabelRules; query != nil {
		if err := _q.loadLabelRules(ctx, query, nodes,
			func(n *Project) { n.Edges.LabelRules = []*ProjectLabelRule{} },
			func(n *Project, e *ProjectLabelRule) { n.Edges.LabelRules = append(n.Edges.LabelRules, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadLabelRules(ctx context.Context, query *ProjectLabelRuleQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectLabelRule)) error {
	...
	query.Where(predicate.ProjectLabelRule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.LabelRulesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_label_rules
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_label_rules" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_label_rules" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
