...
type SessionQuery struct {
	...
	withTask     *TaskQuery
	withMessages *SessionMessageQuery
...
func (_q *SessionQuery) QueryTask() *TaskQuery {
...
func (_q *SessionQuery) QueryMessages() *SessionMessageQuery {
	query := (&SessionMessageClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SessionQuery) QueryEvents() *SessionEventQuery {
	query := (&SessionEventClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SessionQuery) WithTask(opts ...func(*TaskQuery)) *SessionQuery {
	query := (&TaskClient{config: _q.config}).Query()
...
func (_q *SessionQuery) WithMessages(opts ...func(*SessionMessageQuery)) *SessionQuery {
	query := (&SessionMessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SessionQuery) WithEvents(opts ...func(*SessionEventQuery)) *SessionQuery {
	query := (&SessionEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(session.FieldProviderSessionKey).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Session.Query().
//		Select(session.FieldProviderSessionKey).
//		Scan(ctx, &v)
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withTask; query != nil {
		if err := _q.loadTask(ctx, query, nodes, nil,
			func(n *Session, e *Task) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMessages; query != nil {
		if err := _q.loadMessages(ctx, query, nodes,
			func(n *Session) { n.Edges.Messages = []*SessionMessage{} },
			func(n *Session, e *SessionMessage) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Session) { n.Edges.Events = []*SessionEvent{} },
			func(n *Session, e *SessionEvent) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Session, init func(*Session), assign func(*Session, *Task)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Session)
	for i := range nodes {
		if nodes[i].task_sessions == nil {
			continue
		}
		fk := *nodes[i].task_sessions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_sessions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SessionQuery) loadMessages(ctx context.Context, query *SessionMessageQuery, nodes []*Session, init func(*Session), assign func(*Session, *SessionMessage)) error {
	...
	query.withFKs = true
	query.Where(predicate.SessionMessage(func(s *sql.Selector) {
...
func (_q *SessionQuery) loadEvents(ctx context.Context, query *SessionEventQuery, nodes []*Session, init func(*Session), assign func(*Session, *SessionEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.session_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
