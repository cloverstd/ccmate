...
type PromptTemplateQuery struct {
	...
	withLabelRules *ProjectLabelRuleQuery
...
// QueryLabelRules chains the current query on the "label_rules" edge.
func (_q *PromptTemplateQuery) QueryLabelRules() *ProjectLabelRuleQuery {
	query := (&ProjectLabelRuleClient{config: _q.config}).Query()
...
func (_q *PromptTemplateQuery) Clone() *PromptTemplateQuery {
	...
	return &PromptTemplateQuery{
		...
		withLabelRules: _q.withLabelRules.Clone(),
...
// WithLabelRules tells the query-builder to eager-load the nodes that are connected to
// the "label_rules" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PromptTemplateQuery) WithLabelRules(opts ...func(*ProjectLabelRuleQuery)) *PromptTemplateQuery {
	query := (&ProjectLabelRuleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withLabelRules = query
	return _q
}
...
//		GroupBy(prompttemplate.FieldName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.PromptTemplate.Query().
//		Select(prompttemplate.FieldName).
//		Scan(ctx, &v)
...
func (_q *PromptTemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PromptTemplate, error) {
	...
	if query := _q.withLabelRules; query != nil {
		if err := _q.loadLabelRules(ctx, query, nodes,
			func(n *PromptTemplate) { n.Edges.LabelRules = []*ProjectLabelRule{} },
			func(n *PromptTemplate, e *ProjectLabelRule) { n.Edges.LabelRules = append(n.Edges.LabelRules, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PromptTemplateQuery) loadLabelRules(ctx context.Context, query *ProjectLabelRuleQuery, nodes []*PromptTemplate, init func(*PromptTemplate), assign func(*PromptTemplate, *ProjectLabelRule)) error {
	...
	query.withFKs = true
	query.Where(predicate.ProjectLabelRule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(prompttemplate.LabelRulesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.prompt_template_label_rules
		if fk == nil {
			return fmt.Errorf(`foreign-key "prompt_template_label_rules" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "prompt_template_label_rules" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
