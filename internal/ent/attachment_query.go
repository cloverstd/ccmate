...
type AttachmentQuery struct {
	...
	withTask    *TaskQuery
	withMessage *SessionMessageQuery
	withFKs     bool
...
func (_q *AttachmentQuery) QueryMessage() *SessionMessageQuery {
...
func (_q *AttachmentQuery) WithMessage(opts ...func(*SessionMessageQuery)) *AttachmentQuery {
	query := (&SessionMessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if _q.withTask != nil || _q.withMessage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withMessage; query != nil {
		if err := _q.loadMessage(ctx, query, nodes, nil,
			func(n *Attachment, e *SessionMessage) { n.Edges.Message = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AttachmentQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Task)) error {
	...
	for i := range nodes {
		if nodes[i].task_attachments == nil {
			continue
		}
		fk := *nodes[i].task_attachments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AttachmentQuery) loadMessage(ctx context.Context, query *SessionMessageQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *SessionMessage)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Attachment)
	for i := range nodes {
		if nodes[i].session_message_attachments == nil {
			continue
		}
		fk := *nodes[i].session_message_attachments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(sessionmessage.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_message_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
