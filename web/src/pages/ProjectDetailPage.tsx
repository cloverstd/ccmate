...
export default function ProjectDetailPage() {
  ...
  const [projectForm, setProjectForm] = useState({ default_branch: 'main', auto_mode: false, default_agent_profile_id: '', review_agent_profile_id: '', default_prompt_template_id: '', review_prompt_template_id: '', prompt_template_scope: 'project_only' as string })
  ...
  const buildProjectPayload = (overrides?: Record<string, string | boolean>) => {
    ...
    return {
      ...
      review_prompt_template_id: form.review_prompt_template_id ? parseInt(form.review_prompt_template_id as string, 10) : null,
  ...
  useEffect(() => {
    ...
    setProjectForm({
      ...
      review_prompt_template_id: project.review_prompt_template_id != null ? String(project.review_prompt_template_id) : '',
      ...
      <div className="page-head">
        <div style={{ minWidth: 0 }}>
          ...
          <div className="page-sub">
            <a href={project.repo_url} target="_blank" rel="noopener noreferrer">{project.repo_url.replace(/^https?:\/\//, '')}</a>
          </div>
        </div>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <Select value={projectForm.review_prompt_template_id} onChange={(e) => setProjectForm({ ...projectForm, review_prompt_template_id: e.target.value })} className="w-full">
                  <option value="">None (review agent runs with no system prompt)</option>
                  {(templates || []).filter(t => t.project_id === projectId || t.project_id == null).map((t) => (
                    <option key={t.id} value={String(t.id)}>{t.name}{t.project_id == null ? ' (global)' : ''}</option>
                  ))}
                </Select>
                <p className="text-xs text-gray-500 mt-1">Only the template's system_prompt is used; task_prompt is always generated by the reviewer.</p>
              </div>
            </div>
          </CardContent>
          <CardFooter>
